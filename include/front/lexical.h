...
// we should distinguish the operator, so we need a operator table here
extern std::set<std::string> operaters;
// 将识别到的关键字转换为Token，返回类型为TokenType
extern TokenType get_key_TokenType(std::string);
// 将识别到的操作符转换为Token，返回类型为TokenType
extern TokenType get_op_TokenType(std::string);
// 判断是否为数字
...
struct Scanner {
    ...
    std::string preprocess(std::string str, bool &pass);
private:
